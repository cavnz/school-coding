...
const PLAYER_WIDTH = 40;          // Size of your character (try 20 or 50)
const PLAYER_HEIGHT = 80;
// ==== Visual Settings ====
// 📋 TODO: change the colors

const PLAYER_COLOR = 'rgb(52, 152, 219)';  // Color of player (try 'rgb(231, 76, 60)' or 'rgb(46, 204, 113)')
const PLATFORM_COLOR = 'rgb(149, 65, 166)'; // Color of platforms (try 'rgb(52, 73, 94)' or 'rgb(127, 140, 141)')
const BACKGROUND_COLOR = 'rgb(112, 73, 94)'; // Color of background (try 'rgb(44, 62, 80)' or 'rgb(26, 26, 26)')
...
const COIN_SIZE = 30;            // Size of coins
...
function onDeath() {
  ...
  screenShake(10)
  beep(200, 100, 100)
  spawnParticles(player.x + player.width / 2, player.y + player.height / 2, BAD_PLATFORM_COLOR, 30)
...
function drawPlayer() {
  ...
  ctx.beginPath();
  ctx.arc(player.x + 20, player.y + 13, 10, 0, Math.PI * 2);
  ctx.fill();

  // Pupils
  ctx.fillStyle = 'rgb(44, 62, 80)';
  ctx.beginPath();
  ctx.arc(player.x + 20, player.y + 13, 5, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.lineWidth = 5;
  ctx.beginPath();
  ctx.arc(player.x + player.width / 2, player.y + 18, 15, 0.2, Math.PI - 0.2);
  ctx.stroke();
...
function onCoinCollected(coin) {
  ...
  spawnParticles(coin.x, coin.y, COIN_COLOR, 10)
