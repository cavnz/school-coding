...
const PLAYER_COLOR = 'black';  // Color of player (try 'rgb(231, 76, 60)' or 'rgb(46, 204, 113)')
const PLATFORM_COLOR = 'black'; // Color of platforms (try 'rgb(52, 73, 94)' or 'rgb(127, 140, 141)')
const BACKGROUND_COLOR = 'white'; // Color of background (try 'rgb(44, 62, 80)' or 'rgb(26, 26, 26)')
...
const NUMBER_OF_COINS = 100;       // How many coins to spawn (try 3, 10, 20!)
const POINTS_PER_COIN = getRandomIntBetween(1, 100);     // How many points per coin (try 1, 100, 1000!)
const COIN_SIZE = 20;            // Size of coins
const COIN_COLOR = 'BLACK';    // Color of coins (try 'rgb(241, 196, 15)' or 'rgb(230, 126, 34)')
...
const BAD_PLATFORM_COLOR = 'BLACK';  // Color of dangerous platforms (try 'rgb(192, 57, 43)' or 'rgb(0, 0, 0)')
...
const SCORE_COLOR = 'WHITE';    // Color of score text (try 'rgb(241, 196, 15)' or 'rgb(46, 204, 113)')
...
function drawPlayer() {
  ...
  ctx.fillStyle = 'black';
  ctx.beginPath();
  ctx.arc(player.x + 7, player.y + 10, 4, 0, Math.PI * 2);  // Right eye
  ctx.fill();
  ctx.fillStyle = 'black';
  ctx.beginPath();
  ctx.arc(player.x + 17, player.y + 10, 4, 0, Math.PI * 2);  // Right eye
  ctx.fill();
...
function drawScore() {
  ...
  ctx.font = `bold ${SCORE_SIZE}px C`;
  ctx.textAlign = 'center';  // Center the text
  ctx.textBaseline = 'middle';

  // Draw at center of canvas, near the bottom
  ctx.fillText(`your score is ${score}`, canvas.width / 2, SCORE_Y_POSITION);
}
...
function drawBadPlatforms() {
  ...
  for (const platform of badPlatforms) {
    ...
    const spikeWidth = 40;  // How wide each spike is
...
function beep(frequency = 440, duration = 100, volume = 0.3, delay = 0) {
  ...
  try {
    ...
    oscillator.type = 'sine'; // Try: 'sine', 'square', 'sawtooth', 'triangle'
